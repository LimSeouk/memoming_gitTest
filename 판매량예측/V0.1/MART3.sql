...
-- 1. 판매량
CREATE TABLE SULIM_SALE_CNT AS
SELECT /*+ PARALLEL(8) FULL(A) */
       SALE_YYMM,SLPART_CD,PRDG_CD,SALE_PRM_CD,RTFEE, COUNT(1) CNT
  FROM WJM.TW_CT_SALE_MST_MM A
 WHERE 1=1
--   AND SALE_YYMM = '201907'
   AND PORD_YN = 'X'
   AND TOVR_SALE_YN <> 'X'
 GROUP BY SALE_YYMM,SLPART_CD,PRDG_CD,SALE_PRM_CD,RTFEE
;

WITH VW_TMP AS (
SELECT PRDG_CD, SUM(CNT)
  FROM SULIM_SALE_CNT
...
SELECT *
  FROM VW_TMP A
      ,VW_TMP2 B
 WHERE A.PRDG_CD = B.PRDG_CD
 ORDER BY 2 DESC;

CREATE TABLE SULIM_SALT_CNT AS
SELECT /*+ PARALLEL(8) FULL(A) */
       SALE_YYMM,SLPART_CD,PRDG_CD,SALE_PRM_CD,RTFEE, COUNT(1) CNT
  FROM WJM.TW_CT_SALE_MST_MM A
 WHERE 1=1
...
SELECT /*+ PARALLEL(8) FULL(A) */
 ...
 WHERE PROD_DAY = TO_CHAR(ADD_MONTHS(TO_DATE(PROD_YM,'YYYYMM'),1)-1,'DD')
   AND RTN_CONF_DT BETWEEN '19900101' AND '99991231'
   AND CONTRACT_ID   = '2'
   AND VALUATION_CK1 = 'X'
 GROUP BY PROD_YM
 ORDER BY PROD_YM
...
SELECT /*+ PARALLEL(8) FULL(A) */ 
      ...
      ,COUNT(1) CNT 
  FROM TO_ZSDT1048 A
 WHERE YYYY_MM = FIRST_YM
 GROUP BY YYYY_MM
;

SELECT *
  FROM SULIM_OWNE_CNT ORDER BY 1 DESC;

SELECT *
  FROM TO_ZSDT1048
 WHERE YYYY_MM = '201907';
...
-- 2. 최근3개월 평균 순주문 증가율
CREATE TABLE SULIM_INDEX_AVG_PORD_INCR_PCNT AS
WITH VW_TMP AS (
SELECT SALE_YYMM, SUM(CNT) PORD_CNT
  FROM SULIM_SALE_CNT
 GROUP BY SALE_YYMM
